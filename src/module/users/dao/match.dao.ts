...
export class MatchDao {
    ...
    async findpetformatch(reqmatchDto: ReqMatchDto): Promise<ResPetMatchDto[]> {
        try {
            ...
            WHERE p.id_user != ? AND (pmi1.id_userguest IS NULL OR pmi1.id_userguest != ?) AND (pmi2.id_userhome IS NULL OR pmi2.id_userhome != ?)  
            ORDER BY p.id_pet ASC;`;
            const results: ResPetMatchDto[] = await this.petRepository.query(query, [reqmatchDto.id_user, reqmatchDto.id_userguest, reqmatchDto.id_userhome]);
